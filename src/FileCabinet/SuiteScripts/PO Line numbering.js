...
define(['N/error', 'N/record', 'N/runtime', 'N/format', 'N/redirect', 'N/url', 'N/email',
    ...
    function beforeSubmit(context) {
        ...
        if (context.type == context.UserEventType.EDIT) {
            ...
            log.debug('untouched maxValue', maxValue);
            if (isEmpty(maxValue)) {
                maxValue = currentRecord.getLineCount({sublistId: 'item'});
                log.debug('touched maxValue', maxValue);
            }
          	log.debug("Getting Maximum Value", maxValue);
            
            var existingLineNums = [];
            var emptyLines = [];
          
            /* Get empty lines & existing line numbers */
            for (j = 0; j < totalitemCount; j++) {
                ...
                existingLineNums.push(emptylineid);
                log.debug("Empty Lines Length", isEmpty(emptylineid.length));
                if (isEmpty(emptylineid)) {
                  	emptyLines.push(j);
                }
            }
            for (j = 0; j < emptyLines.length; j++) {
                while (existingLineNums.indexOf(maxValue) >= 0) maxValue++;
